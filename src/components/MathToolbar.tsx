...
import type { SymbolCategories, MathSymbol } from '../types';

const symbolCategories: SymbolCategories = {
  operations: {
    label: 'Opérations',
    icon: '±',
    symbols: [
...
const MathToolbar = ({ onInsert }: MathToolbarProps) => {
  ...
  const filteredSymbols = useMemo(() => {
    if (!activeCategory || !searchQuery) {
      return activeCategory ? symbolCategories[activeCategory].symbols : [];
    }

    const query = searchQuery.toLowerCase();
    return symbolCategories[activeCategory].symbols.filter(
      (symbol) =>
      ...
      {activeCategory && (
        <div className="symbol-palette">
          <div className="palette-header">
            <span>{symbolCategories[activeCategory].label}</span>
